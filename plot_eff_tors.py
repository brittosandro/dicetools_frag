...
if __name__ == '__main__':
  ...
  parser.add_argument("--shiftinterval", help="calculates data within a range of the entered reference angle. ", action="store_true")
  parser.add_argument("-i", "--interphi", type=int, help="range angle")
  ...
  if args.shiftinterval:
      #this is the final angle in radians of the desired range.
      sup_phi = refphi + args.interphi * np.pi / 180
      inf_phi = refphi - args.interphi * np.pi / 180

      print("# Reference angle in radians:  %f" % refphi)
      print("# Initial angle (degrees): %f" % (180.*refphi/np.pi))
      print("# Angles (degrees). Upper limit: %f" %((180.*refphi/np.pi)+(args.interphi)))
      print("# Angles (degrees). Inferior limit: %f " %((180.*refphi/np.pi)-(args.interphi)))
  else:
      print("# Initial angle (degrees): %f" % (180.*refphi/np.pi))

  # calculate which points will be considered
  if args.shiftinterval:
      sup_points = np.linspace(refphi, sup_phi, args.npoints)
      inf_points = np.linspace(refphi, inf_phi, args.npoints)
      points = np.concatenate((inf_points, sup_points))
  else:
  ...
  # convert to degrees and put it in [0,360) or in [-180,180) or in (-interval, interval)
